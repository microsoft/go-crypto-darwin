...
func TestRSAKeyGeneration(t *testing.T) {
	for _, size := range []int{2048, 3072} {
		t.Run(strconv.Itoa(size), func(t *testing.T) {
			t.Parallel()
			_, _, _, _, _, _, _, _, err := commoncrypto.GenerateKeyRSA(size)
			if err != nil {
				t.Fatal(err)
			}
		})
...
func testRSAEncryptDecryptPKCS1(t *testing.T, priv *commoncrypto.PrivateKeyRSA, pub *commoncrypto.PublicKeyRSA) {
	msg := []byte("hi!")
	enc, err := commoncrypto.EncryptRSAPKCS1(pub, msg)
	if err != nil {
		t.Fatalf("EncryptPKCS1v15: %v", err)
	}
	dec, err := commoncrypto.DecryptRSAPKCS1(priv, enc)
	if err != nil {
...
func TestRSAEncryptDecryptOAEP(t *testing.T) {
	...
	enc, err := commoncrypto.EncryptRSAOAEP(sha256, pub, msg, label)
	if err != nil {
		t.Fatal(err)
	}
	dec, err := commoncrypto.DecryptRSAOAEP(sha256, priv, enc, label)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(dec, msg) {
	...
	_, err = commoncrypto.DecryptRSAOAEP(sha1, priv, enc, label)
	if err == nil {
...
func TestRSAEncryptDecryptOAEP_WrongLabel(t *testing.T) {
	...
	msg := []byte("hi!")
	priv, pub := newRSAKey(t, 2048)
	enc, err := commoncrypto.EncryptRSAOAEP(sha256, pub, msg, []byte("ho!"))
	if err != nil {
		t.Fatal(err)
	}
	dec, err := commoncrypto.DecryptRSAOAEP(sha256, priv, enc, []byte("wrong!"))
	if err == nil {
...
func TestRSASignVerifyPKCS1v15(t *testing.T) {
	...
	signed, err := commoncrypto.SignRSAPKCS1v15(priv, crypto.SHA256, hashed)
	if err != nil {
		t.Fatal(err)
	}
	err = commoncrypto.VerifyRSAPKCS1v15(pub, crypto.SHA256, hashed, signed)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestRSASignVerifyPKCS1v15_Unhashed(t *testing.T) {
	msg := []byte("hi!")
	priv, pub := newRSAKey(t, 2048)
	signed, err := commoncrypto.SignRSAPKCS1v15(priv, 0, msg)
	if err != nil {
		t.Fatal(err)
	}
	err = commoncrypto.VerifyRSAPKCS1v15(pub, 0, msg, signed)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestRSASignVerifyPKCS1v15_Invalid(t *testing.T) {
	...
	err = commoncrypto.VerifyRSAPKCS1v15(pub, crypto.SHA256, msg, signed)
	if err == nil {
...
func TestRSASignVerifyRSAPSS(t *testing.T) {
	...
	for i, test := range saltLengthCombinations {
		signed, err := commoncrypto.SignRSAPSS(priv, crypto.SHA256, hashed, test.signSaltLength)
		if err != nil {
			t.Errorf("#%d: error while signing: %s", i, err)
			continue
		}
		err = commoncrypto.VerifyRSAPSS(pub, crypto.SHA256, hashed, signed, test.verifySaltLength)
...
func newRSAKey(t *testing.T, size int) (*commoncrypto.PrivateKeyRSA, *commoncrypto.PublicKeyRSA) {
	t.Helper()
	N, E, D, P, Q, Dp, Dq, Qinv, err := commoncrypto.GenerateKeyRSA(size)
	if err != nil {
...
func newRSAKeyFromParams(t *testing.T, N, E, D, P, Q, Dp, Dq, Qinv commoncrypto.BigInt) (*commoncrypto.PrivateKeyRSA, *commoncrypto.PublicKeyRSA) {
	t.Helper()
	priv, err := commoncrypto.NewPrivateKeyRSA(N, E, D, P, Q, Dp, Dq, Qinv)
	if err != nil {
		t.Fatalf("NewPrivateKeyRSA: %v", err)
	}
	pub, err := commoncrypto.NewPublicKeyRSA(N, E)
	if err != nil {
...
func BenchmarkEncryptRSAPKCS1(b *testing.B) {
	...
	test2048PubKey, err := commoncrypto.NewPublicKeyRSA(bbig.Enc(n), bbig.Enc(big.NewInt(3)))
	if err != nil {
		b.Fatal(err)
	}
	b.StartTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		if _, err := commoncrypto.EncryptRSAPKCS1(test2048PubKey, []byte("testing")); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkGenerateKeyRSA(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_, _, _, _, _, _, _, _, err := commoncrypto.GenerateKeyRSA(2048)
		if err != nil {
			b.Fatal(err)
		}
	}
