...
func TestHKDFSHA3(t *testing.T) {
	...
	// The inputs are RFC 5869 Test Case 1. RFC 5869 does not publish SHA-3
	// vectors, so the expected PRK and OKM below were generated with Go's
	// crypto/hkdf over crypto/sha3 and cross-checked against CryptoKit.
	master := []byte{
		0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
		0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
		0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			prk, err := xcrypto.ExtractHKDF(tt.hash, master, salt)
			if err != nil {
				t.Fatalf("error extracting HKDF: %v", err)
			}
			if !bytes.Equal(prk, tt.prk) {
			...
			out, err := xcrypto.ExpandHKDF(tt.hash, prk, info, len(tt.out))
			if err != nil {
